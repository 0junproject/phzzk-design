...
## 3. 시청자 팔로우 및 실시간 카운트 시나리오 (Viewer - Follow)

**[진행 흐름]**
1. **[시청자]** 특정 스트리머의 채널에 접속하여 '팔로우' 버튼을 클릭한다.
2. **[시스템]** 게이트웨이가 요청을 받아 유저 서비스(또는 팔로우 도메인)로 라우팅한다.
3. **[시스템]** DB의 Row Lock 및 병목 방지를 위해 RDBMS에 즉시 쓰지 않고, Redis의 스트리머 팔로워 카운터(In-memory)를 1 증가(`INCR`)시킨다.
4. **[시스템]** 클라이언트(시청자)에게 즉시 '팔로우 성공' 응답을 반환하여 UI를 업데이트한다. (응답 지연시간 최소화)
5. **[시스템]** 동시에 메시지 큐(Kafka/RabbitMQ)에 `[팔로우_이벤트]`를 비동기로 발행(Publish)한다.
6. **[시스템]** 백엔드 데이터 동기화 워커(Worker)가 큐에서 이벤트를 소비(Consume)하여, 일정 주기나 묶음(Chunk) 단위로 RDBMS의 팔로우 관계 테이블에 일괄 저장(Batch Insert)하여 영속성을 보장한다.